"""Bank Jobs question crawler (Bangladesh Bank, Sonali Bank, etc.)."""
...
logger = get_logger("bank_crawler")
...
class BankCrawler:
    ...
    def crawl(self, selected_sources: list[str] | None = None) -> list[dict]:
        ...
        for idx, target in enumerate(targets, 1):
            ...
            logger.info(f"[Bank] Starting {target['name']} — {target['url']}")
            try:
                ...
                logger.error(f"[Bank] Error crawling {target['name']}: {exc}")
                self.pq.put("error", self.EXAM_TYPE, f"Error: {target['name']} — {exc}")

        self.pq.put("log", self.EXAM_TYPE, f"Bank crawl done. Total raw pages: {len(records)}")
        return records
    ...
    def _crawl_site(self, target: dict) -> list[dict]:
        ...
        logger.info(f"[Bank] Found {len(sub_links)} links on {target['name']}")
    ...
    def _find_question_links(self, html: str, base_url: str) -> list[str]:
        ...
        links = []
        keywords = ["bank", "question", "exam", "mcq", "written", "paper",
                    "solution", "recruitment", "circular", "প্রশ্ন"]
